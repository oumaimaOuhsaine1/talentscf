...
export default function PreDiagnosticsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      const response = await fetch('http://127.0.0.1:5000/api/contact/pre-diagnostic', {
        method: 'POST',
      ...
      if (response.ok) {
        setSubmitted(true)
        setForm({ nom: '', prenom: '', email: '', telephone: '', organisation: '', secteur: '', urgence: '', description: '' })
      ...
      <main className="flex-1">
        <section className="relative h-56 w-full overflow-hidden bg-gradient-to-r from-[#003E6B] to-[#002845]">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4">
            <h1 className="text-3xl md:text-4xl font-extrabold text-white">Pré-diagnostics de besoins</h1>
            <p className="text-white/80 mt-2 max-w-2xl">Décrivez vos besoins et nous reviendrons avec une proposition.</p>
          </div>
        </section>
                ...
                <div className="bg-white dark:bg-gray-800 border border-border rounded-xl p-10 shadow-sm">
                  {submitted ? (
                    <div className="flex items-center gap-4 p-4 rounded-md bg-green-50 border border-green-200 text-green-800">
                      ...
                      <div>
                        <div className="font-semibold">Votre pré-diagnostic a été envoyé</div>
                        <div className="text-sm">Nous analyserons votre demande et reviendrons rapidement.</div>
                      </div>
                    </div>
                  ) : null}
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6 mt-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div>
                        <label className="block text-sm font-medium mb-2">Organisation (optionnel)</label>
                        <input name="organisation" value={form.organisation} onChange={handleChange} className="w-full px-5 py-4 border border-border rounded-lg bg-card focus:outline-none focus:ring-2 focus:ring-[#005A9C] text-base" />
                      </div>

                      <div>
                        <label className="block text-sm font-medium mb-2">Secteur d'activité</label>
                        <input name="secteur" value={form.secteur} onChange={handleChange} className="w-full px-5 py-4 border border-border rounded-lg bg-card focus:outline-none focus:ring-2 focus:ring-[#005A9C] text-base" />
                      </div>

                      <div>
                        <label className="block text-sm font-medium mb-2">Degré d'urgence</label>
                        <select name="urgence" value={form.urgence} onChange={handleChange} className="w-full px-5 py-4 border border-border rounded-lg bg-card focus:outline-none focus:ring-2 focus:ring-[#005A9C] text-base">
                      ...
                      <div className="md:col-span-2">
                        ...
                        <textarea name="description" value={form.description} onChange={handleChange} rows={6} className="w-full px-5 py-4 border border-border rounded-lg bg-card focus:outline-none focus:ring-2 focus:ring-[#005A9C] text-base resize-none" />
                      </div>
                    </div>
